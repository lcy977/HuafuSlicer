    ...
    std::array<std::mutex, 64>                       &lines_mutex)
{
    ...
    auto make_intersection_line = [](const Point &a, const Point &b,
                                     int a_id, int b_id,
                                     int edge_a_id, int edge_b_id,
                                     IntersectionLine::FacetEdgeType edge_type) -> IntersectionLine {
        IntersectionLine il;
        il.a         = a;
        il.b         = b;
        il.a_id      = a_id;
        il.b_id      = b_id;
        il.edge_a_id = edge_a_id;
        il.edge_b_id = edge_b_id;
        il.edge_type = edge_type;
        return il;
    };
            ...
            if (size_t line_id = ProjectionFromTop ? slice_id : slice_id + 1; ProjectionFromTop || line_id < lines.at_slice.size())
                ...
                for (int iedge = 0; iedge < 3; ++ iedge)
                    if (facet_neighbors(iedge) == -1) {
                        ...
                        IntersectionLine il = make_intersection_line(
                            Point(to_2d(vertices[i]).cast<coord_t>()),
                            Point(to_2d(vertices[j]).cast<coord_t>()),
                            indices(i), indices(j),
                            -1, -1,
                ...
                for (int iedge = 0; iedge < 3; ++ iedge)
                    if (facet_neighbors(iedge) == -1) {
                        ...
                        if (intersects_this && intersects_prev) {
                            ...
                            emit_slab_edge(
                                make_intersection_line(
                                    Point(to_2d(edge_up ? vertices[i] : vertices[j]).cast<coord_t>()),
                                    (il.edge_a_id == edge_id ? il.a : il.b),
                                    edge_up ? indices(i) : indices(j), -1,
                                    -1, edge_id + num_edges,
                                    IntersectionLine::FacetEdgeType::Slab),
                                slab_id, ProjectionFromTop != edge_up);
                            ...
                            emit_slab_edge(
                                make_intersection_line(
                                    ...
                                    Point(to_2d(edge_up ? vertices[j] : vertices[i]).cast<coord_t>()),
                                    -1, edge_up ? indices(j) : indices(i),
                                    edge_id, -1,
                                    IntersectionLine::FacetEdgeType::Slab),
                                slab_id, ProjectionFromTop != edge_up);
